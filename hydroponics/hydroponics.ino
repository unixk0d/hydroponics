...
const int sensor01 = 2;//   гидропоника - датчик верхнего уровня btnUp
const int dev01 = 3;//      гидропоника - клапан сброса воздуха по умолчанию открыт, 2.7 вольта
const int dev02 = 4;//      гидропоника - помпа, нагнетающая давление, активна совместно с клапаном сброса воздуха, но не по его расписанию, 12 вольт
const int dev03 = 5;//      гидропоника - помпа аэрации, 12 вольт
const int dev04 = 6;//      гидропоника - 220 вольт (лампа теплого света)
...
const int dev08 = 10;//     холл - обход счетчика
const int dev13 = 13;//     L светодиод  активируется в случае неисправности
...
boolean swStream = false; //бинарный переключатель состояния, отключает все не связанное со стримом, мешающее внешне.
...
void GetEthernet(){
  ...
  if (vol = ethernet.serviceRequest())
  {
    ...
    if(vol == "stat"){ 
      ...
      ethernet.print(swDev03);
      ethernet.respond();
      delay(4000); // отсечка пакетов, производящих паразитный инкремент
    }  
...
// ,также для устранения постоянной переинициализации переменной времени.
  if ((volTimeDow == 1
     ||volTimeDow == 2
     ||volTimeDow == 3
     ||volTimeDow == 4
...
// ,также для устранения постоянной переинициализации переменной времени.
  ...
  if ((volTimeDow == 5
     ||volTimeDow == 6)&&((volTimeHour == 23) && volTimeMin == 59 && volTimeSec == 59))//подготавливаем гидропонику для выходных дней
                         {
                           swDev01  = false;
                           swDev02  = false;
                           swDev03  = false;
...
// sensor 01
  ...
  {
    statSensor01 = true;//если на пине появился высокий уровень, то  датчик ушел в обрыв, и значит ПЫЗДЕЦ
  }
